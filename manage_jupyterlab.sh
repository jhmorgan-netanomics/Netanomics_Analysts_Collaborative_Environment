...
# Defaults
DEFAULT_CONTAINER_NAME="collaborative-env-jupyter"
...
echo "Managing JupyterLab for container: $CONTAINER_NAME on IP: $IP and port: $PORT"

# Check if the port is in use and kill the associated process
check_and_free_port() {
...
if [ -n "$RUNNING_CONTAINER" ]; then
    ...
    echo "Attaching to the container to start JupyterLab..."
    docker exec -it "$CONTAINER_NAME" /usr/local/bin/start_workspace.sh jupyter
    exit 0
fi
...
docker run -itd -p "$PORT:8888" -p 8080:8080 --name "$CONTAINER_NAME" collaborative-env jupyter

# Verify the container creation and attach to it
if [ $? -eq 0 ]; then
